  ...
  try {
    ...
    if (existingUsername) {
      return res
        .status(400)
        .send(
          "User with this username already exists. Please enter a unique one."
        );
    }
    ...
    if (
      ...
      registerInput.password.length > 12
    ) {
      return res
        .status(400)
        .send("Password must be between 8 and 12 characters long.");
    }
...
module.exports.loginUser = async (req, res) => {
  ...
  const userCredential = await userModel.findOne({
    username: loginInput.username,
  });

  if (userCredential == null) {
    res.send({ code: 400, message: "User doesn't exist!!!" });
  } else {
    try {
      const isMatch = await bcrypt.compare(
        loginInput.password,
        userCredential.password
      ); // Compare the input plain-text password with stored hashed-password.
      if (isMatch) {
        jwt.sign(
      ...
      } else res.send({ code: 404, message: "Password Incorrect!" });
    } catch (err) {
      console.log(err);
      res.send({ code: 400, message: "Login Error!" });
    }
  }
};
...
module.exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!user.verified) {
      return res
        .status(400)
        .send("Email hasn't been verified. Please check your inbox.");
    }
  ...
  try {
    ...
    if (existingOTP && existingOTP.expiresAt > Date.now()) {
      return res.json({
        status: "PENDING",
        message:
          "Please wait for the existing OTP to expire before requesting a new one.",
        data: {
          userId: _id,
          email,
        },
